		...
		for(int i = 0; i < vertices; i++) {
			for(int j = 0; j < vertices; j++) {
				...
				for(int k = 0; k < vertices; k++) {
					// mildly hacky way to convert the booleans into int for calculation
					int boolToInt1 = adjMatrix[k][j] ? 1 : 0;	
					int boolToInt2 = adjMatrix[i][k] ? 1 : 0;	
					matrix[i][j] += boolToInt1 * boolToInt2; 
