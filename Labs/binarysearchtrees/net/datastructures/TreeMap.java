/**
 * Lab Solution begins on Line <b>490</b>
 */
...
public class TreeMap<K,V> extends AbstractSortedMap<K,V> {
   ...
   * implement atIndex(i):  Return the position p of the entry at index i of a sorted map
   * & indexOf(p):          Return the index i of the entry at position p of a sorted map
   * both in <b>O</b>(h) time
   */

  public Position<Entry<K,V>> atIndex(Integer index) {
    /* O(n) solution */
    int counter = 0;
  ...
  public int indexOf(Position<Entry<K,V>> position) {
    int counter = 0; 
    for (Position<Entry<K,V>> entry : tree.inorder()) {
      if (position == entry) 
        return counter;
  ...
  public static void main(String args[]) {
      ...
      System.out.println("" + ": " + test.indexOf(walk));
